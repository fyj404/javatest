...
### 线程池大小
1. cpu密集型，线程会很消耗cpu资源，主要以计算为主，线程池数量=cpu数量+1
2. IO密集型，线程主要是在读写、网络等待消耗时间。数量=cpu数量 *（1+读写时间/计算时间）
8核，网络花200ms，自己计算需要50ms。8*（1+200/50）= 40个线程 
```java
package com.example;
